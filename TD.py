...
def evaluate_TAD(X, M, graph):
    ...
    print("number access: ", nb_acces, "/", M, "|taux d'occupation", nb_acces / M * 100, "%", "|mean: ", mean, "|min: ",
          min, "|max:", max)
...
def hashage_ascii(mot):
    ...
    for lettre in mot:
        ...
        position = + 1
        list_result.append(
            asciiLettre * base ** (longueur - position))  # on ajoute la valeur associée à la lettre dans le liste
